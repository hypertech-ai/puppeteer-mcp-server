...
export async function ensureBrowser(): Promise<Page> {
  if (!browser) {
    logger.info(
      ...
      process.env.DOCKER_CONTAINER ? "docker" : "npx",
    );
    browser = await puppeteer.launch(
      process.env.DOCKER_CONTAINER ? dockerConfig : npxConfig,
    );
...
export async function getDebuggerWebSocketUrl(
  ...
  try {
    ...
    if (!data.webSocketDebuggerUrl) {
      throw new Error(
        "No WebSocket debugger URL found. Is Chrome running with --remote-debugging-port?",
      );
    }
    return data.webSocketDebuggerUrl;
  } catch (error) {
    throw new Error(
      `Failed to connect to Chrome debugging port ${port}: ${(error as Error).message}`,
    );
  }
}
